    ...
    def execute(self):
        if self.is_first_call:
            ...
            if self.state_dict["last_received_sentence"] is not None:
                entities = self.container.ner.get_entities(self.state_dict["last_received_sentence"])
                """
                Task decision tree (bad description)
                """
...
"""Figure out some console I/O here"""
class WaitForResponseState(State):
    ...
    def execute(self):
        ...
        got_new_sentence = False
        rospy.loginfo("Please input response in console:")
        response = str(input())
        rospy.loginfo("Your response was: ")
        rospy.loginfo(response)
        self.state_dict["last_received_sentence"] = response
        got_new_sentence = True
...
class DialogFlow:
    def __init__(self, ner_model_path, ner_tag_path, feature_weights_path, db_path, background_image_file, debug=False):
...
if __name__ == '__main__':
    ...
    try:
        dialog = DialogFlow(ner_model_path=args.ner_model_path, ner_tag_path=args.tags_path,
                            ...
                            background_image_file=args.background_image, debug=True)
        dialog.start()
